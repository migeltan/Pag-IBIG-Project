...
public class MemberInfoForm extends JFrame {
    ...
    private void prefillFromDb() {
        ...
        setText(birthdateField,            existing.getBirthdate() != null ? existing.getBirthdate().toString() : "");
        ...
        setText(employeeNumberField,       existing.getEmployeeNumber() != null ? String.valueOf(existing.getEmployeeNumber()) : "");
        ...
        setText(allowBasicField,       existing.getAllowBasic() != null ? existing.getAllowBasic().toPlainString() : "");
        setText(allowOtherSourcesField, existing.getAllowOtherSources() != null ? existing.getAllowOtherSources().toPlainString() : "");
        setText(totalMoIncomeField,    existing.getTotalMoIncome() != null ? existing.getTotalMoIncome().toPlainString() : "");
        ...
        // ── Membership Type — value may now be a free-text "Others" value ─────
        String dbType = existing.getMembershipType();
        String uiType = fromDbMembershipType(dbType);
        if ("Others".equals(uiType)) {
            // It's a custom/unknown value — show it in the Others text field
            setCombo(membershipTypeBox, "Others");
            setText(membershipTypeOthersField, dbType);
            ...
            setCombo(membershipTypeBox, uiType);
        }

        // ── Membership Category — same logic ──────────────────────────────────
        String dbCat = existing.getMembershipCategory();
        String uiCat = fromDbMembershipCategory(dbCat);
        if ("Other Earning Groups".equals(uiCat)) {
            setCombo(membershipCategoryBox, "Other Earning Groups");
            // If the stored value is literally "OTHER EARNING GROUPS", no extra text needed.
            // But if it was something custom, show it:
            if (!"OTHER EARNING GROUPS".equalsIgnoreCase(dbCat)) {
                setText(membershipCategoryOtherEarningField, dbCat);
            }
            ...
            setCombo(membershipCategoryBox, uiCat);
        }
    ...
    private void handleSave() {
        if (isBlank(memberNameField) || isBlank(birthdateField) || isBlank(birthplaceField)
                || isBlankPhone(cellphoneNumField) || isBlank(presentHomeAddressField)
                || isBlank(permanentHomeAddressField) || isBlank(allowBasicField)
                || isComboDefault(occupationalStatusBox) || isComboDefault(membershipTypeBox)
                || isComboDefault(membershipCategoryBox) || isComboDefault(maritalStatusBox)
                || isComboDefault(sexBox) || isComboDefault(citizenshipBox)
        ...
        // ── Membership Type: if "Others" selected, save the text field value ──
        String selectedType = (String) membershipTypeBox.getSelectedItem();
        String membershipTypeDb;
        if ("Others".equals(selectedType)) {
            String custom = membershipTypeOthersField.getText().trim();
            if (custom.isEmpty()) { showError("Please specify your Membership Type."); return; }
            membershipTypeDb = custom.toUpperCase(); // store free text directly in Membership_Type
        } else {
            membershipTypeDb = toMembershipTypeEnum(selectedType);
        }
        ...
        if ("Filipino".equals(citizenshipBox.getSelectedItem())) {
            ...
            if (citizenship.isEmpty()) { showError("Please specify your citizenship."); return; }
        } else {
        ...
        if (!empNumText.isEmpty()) {
            ...
            catch (NumberFormatException ex) { showError("Employee Number must be a valid number."); return; }
        }

        MemberTable member = new MemberTable(
                mid, occupationalStatus, membershipTypeDb, membershipCatDb,
                memberNameField.getText().trim(), fatherNameField.getText().trim(),
                motherNameField.getText().trim(), spouseNameField.getText().trim(),
                birthdate, maritalStatus, birthplaceField.getText().trim(),
                citizenship, sex, crnField.getText().trim(), frequency,
                tinField.getText().trim(), sssField.getText().trim(), empNumber,
                presentHomeAddressField.getText().trim(), permanentHomeAddressField.getText().trim(),
                mailingAddress, homeTelNumField.getText().trim(), rawPhone,
                busDirectLineField.getText().trim(), busTrunkLineField.getText().trim(),
        ...
        if (!saved) { showError("Failed to save. Please check your connection and try again."); return; }
        if (!recordExists) recordExists = true;

        RegistrationSession session = RegistrationSession.getInstance();
        session.setMemberData(member);
        session.setMemberInfoDone(true);

        JOptionPane.showMessageDialog(this, "Member information saved successfully!", "Success",
    ...
    private JPanel buildContent() {
        ...
        // Row 1: MID | Membership Type | Membership Category
        JPanel r1 = row(3);
        ...
        // "Others" panel for Membership Type — hidden by default
       ...
       membershipTypeBox.addActionListener(e -> {
        ...
        membershipCategoryBox.addActionListener(e -> {
            ...
            membershipCategoryOtherEarningPanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, show ? 70 : 0));
            membershipCategoryOtherEarningPanel.setPreferredSize(new Dimension(0, show ? 70 : 0));
            c.revalidate(); c.repaint();
        });
        ...
        r2.add(lf("Occupational Status *",
                occupationalStatusBox = cb(new String[]{"Select", "Employed", "Unemployed", "First Time Jobseeker"})));
        r2.add(lf("Frequency of Membership Savings *",
                frequencyOfMembershipSavingsBox = cb(new String[]{"Select", "Monthly", "Quarterly", "Semi-Annual", "Annual"})));
        c.add(r2);
        c.add(vgap(26));
        ...
        r7.add(lf("Employee No.", employeeNumberField = tfDigitsOnly(14)));
    ...
    private JTextField tfDate() {
        ...
        field.addFocusListener(new FocusAdapter() {
            @Override public void focusLost(FocusEvent e) {
                ...
                if (error != null) {
                    showError(error);
                    SwingUtilities.invokeLater(() -> { field.setText(""); field.requestFocusInWindow(); });
                }
            }
        });
        return field;
    ...
    private void applyDatePadAndFormat(JTextField f) {
        ...
        if (rest.length() == 1) rest = "0" + rest;
            ...
            if (rest.charAt(0) == '0') rest = rest.substring(0, 2) + "0" + rest.substring(2);
            else rest = "0" + rest;
        }
    ...
    private JTextField tfPhone() {
        ...
        ((javax.swing.text.AbstractDocument) field.getDocument())
            .setDocumentFilter(new javax.swing.text.DocumentFilter() {
                @Override public void replace(FilterBypass fb, int offset, int length, String text,
                    ...
                    String current = fb.getDocument().getText(0, fb.getDocument().getLength());
                    String afterPrefix = current.length() > PREFIX.length() ? current.substring(PREFIX.length()) : "";
        ...
        field.addCaretListener(e -> SwingUtilities.invokeLater(() -> {
            if (field.getCaretPosition() < PREFIX.length()) field.setCaretPosition(PREFIX.length());
        }));
        return field;
    }
        ...
        ((javax.swing.text.AbstractDocument) field.getDocument())
            .setDocumentFilter(new javax.swing.text.DocumentFilter() {
                @Override public void replace(FilterBypass fb, int offset, int length, String text,
                    ...
                    if (newLen <= maxLen) super.replace(fb, offset, length, filtered, attrs);
                }
            });
        return field;
    }
    ...
    // ── DB Enum Mappers ───────────────────────────────────────────────────────
    private String toDbEnum(String v) {
        switch (v) {
            case "Employed":             return "EMPLOYED";
            case "Unemployed":           return "UNEMPLOYED";
    ...
    private String toMailingEnum(String v) {
        switch (v) {
    ...
    private boolean isBlankPhone(JTextField f)     { return f.getText().replaceAll("[^0-9]", "").length() < 10; }
    ...
    private String validateDate(String dateStr) {
        ...
        switch (month) {
            ...
            case 2: boolean leap = (year%4==0&&year%100!=0)||(year%400==0); maxDays = leap?29:28; break;
        ...
        try {
            ...
            if (entered.isAfter(java.time.LocalDate.now())) return "Birthdate cannot be in the future.";
    ...
    private JPanel row(int cols) {
        ...
        p.setMaximumSize(new Dimension(Integer.MAX_VALUE, 70)); return p;
    }
    ...
    private JTextField tf(int maxLen) {
        JTextField field = new JTextField() {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.setColor(isFocusOwner() ? new Color(96,216,164,200) : new Color(255,255,255,50));
        ...
        ((javax.swing.text.AbstractDocument) field.getDocument())
            .setDocumentFilter(new javax.swing.text.DocumentFilter() {
                ...
                @Override public void replace(FilterBypass fb, int offset, int length, String text,
                    ...
                    if (newLen <= maxLen) super.replace(fb, offset, length, text, attrs);
                }
            });
        field.addFocusListener(new FocusAdapter() {
    ...
    private JButton buildButton(String text, Color color) {
        ...
        btn.setPreferredSize(new Dimension(160, 48)); return btn;
    }
